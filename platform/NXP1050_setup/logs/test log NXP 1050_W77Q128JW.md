[MCUXpresso Semihosting Telnet console for 'Compatibility_tests_suite LinkServer Debug' started on port 60751 @ 127.0.0.1]

```
Initializing flash device...
deviceID = 0x17
JEDEC manufacturerID = 0xEF, memoryType = 0x8A, capacity = 0x18 (16384 KB)
Read HW_VER using OP1 and OP2 commands
HW_VER = 0x00172411
Detected Target is W77Q128JW
Octal bus mode is not supported
Read SSR register
SSR = 0xF2000060
SSR Read passed
Device was initialized successfully
Transport test started...
Test Single bus mode: 1s-1s-1s
Frequency is set to 133 MHz
Execute erase sector command
Using opcode 0x20
Execute page program command
Program Pattern 0x5A
Using opcode 0x02
Execute fast read page command
Using opcode 0x0B
Verify Data Pattern 0x5A
Verify Successfully
Test Quad bus mode: 1s-4s-4s
Frequency is set to 133 MHz
Execute fast read page command
Using opcode 0xEB
Verify Data Pattern 0x5A
Verify Successfully
Frequency is set to 133 MHz
Transport test was completed successfully
```